...
namespace orbis
{
	inline namespace logs
	{
		...
		template <>
		struct log_class_string<void*, void>
		{
			static void format(std::string& out, const void* arg);
		};
		...
		template <>
		struct log_class_string<char*, void>
		{
		...
		struct log_class_string<char[N]>
		{
			static void format(std::string& out, const void* arg)
			{
		...
		template <std::size_t N>
		struct log_class_string<const char[N]>
		{
			static void format(std::string& out, const void* arg)
			{
				log_class_string<char*, void>::format_n(out, arg, N);
		...
		struct log_type_info
		{
			...
			template <typename T>
			static constexpr log_type_info make()
			{
