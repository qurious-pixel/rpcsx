	...
	uint64_t paramsSz, uint64_t arg6)
{
	...
	if (op == 0x320)
	{
		...
		{
			if (syncCallParams.method == 0xce110007)
			{ // SceMbusIpcAddHandleByUserId
		...
		else if (client->name == "SceUserService")
		{
			...
			if (syncCallParams.method == 0x30011)
			{ // get initial user id
		...
		{
			if (syncCallParams.method == 0x12340000)
			...
			else if (syncCallParams.method == 0x1234000f)
			{ // create event flag
			...
			else if (syncCallParams.method == 0x12340002)
			{ // something like open
			...
			else if (syncCallParams.method == 0x12340006)
			{ // close port
