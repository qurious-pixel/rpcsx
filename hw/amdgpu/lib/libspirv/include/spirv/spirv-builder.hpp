...
namespace spirv
{
	...
	struct AnyConstantValue : Value
	{
		...
		template <typename T>
		AnyConstantValue(ConstantValue<T> specialization)
		{
		...
		template <typename T>
		explicit operator ConstantValue<T>()
		{
	...
	namespace detail
	{
		...
		template <>
		struct TypeToValueImpl<Type>
		{
			using type = Value;
		};
		template <>
		struct TypeToValueImpl<BoolType>
		{
			using type = BoolValue;
		};
		template <>
		struct TypeToValueImpl<IntType>
		{
			using type = IntValue;
		};
		template <>
		struct TypeToValueImpl<SIntType>
		{
			using type = SIntValue;
		};
		template <>
		struct TypeToValueImpl<UIntType>
		{
			using type = UIntValue;
		};
		template <>
		struct TypeToValueImpl<FloatType>
		{
			using type = FloatValue;
		};
		template <>
		struct TypeToValueImpl<StructType>
		{
			using type = StructValue;
		};
		template <>
		struct TypeToValueImpl<PointerType>
		{
			using type = PointerValue;
		};
		template <>
		struct TypeToValueImpl<VariableValue>
		{
			using type = PointerValue;
		};
		template <>
		struct TypeToValueImpl<VectorType>
		{
			using type = VectorValue;
		};
		template <>
		struct TypeToValueImpl<ArrayType>
		{
			using type = ArrayValue;
		};
		template <>
		struct TypeToValueImpl<SamplerType>
		{
			using type = SamplerValue;
		};
		template <>
		struct TypeToValueImpl<ImageType>
		{
			using type = ImageValue;
		};
		template <>
		struct TypeToValueImpl<SampledImageType>
		{
			using type = SampledImageValue;
		};

		template <typename T>
		struct TypeToValueImpl<PointerToType<T>>
		{
			using type = PointerToValue<T>;
		};
		template <typename T>
		struct TypeToValueImpl<VectorOfType<T>>
		{
			using type = VectorOfValue<T>;
		};

		template <typename T>
		struct TypeToValueImpl<ArrayOfType<T>>
		{
	...
	class BlockBuilder
	{
		...
		template <typename T>
		auto newId() -> decltype(mIdGenerator->newId<T>())
		{
	...
	public:
		...
		BlockBuilder(IdGenerator& idGenerator, Block id,
			...
			, id(id)
			, bodyRegion{expInstructionsCount}
			, terminatorRegion{1}
		{
		}
			...
			std::span<const Id> args = {})
		{
			auto region = bodyRegion.pushOp(
				...
				5 + (operands == spv::ImageOperandsMask::MaskNone ? 0 : 1 + args.size()));
			...
			std::span<const Id> args = {})
		{
			auto region = bodyRegion.pushOp(
				spv::Op::OpImageWrite,
				4 + (operands == spv::ImageOperandsMask::MaskNone ? 0 : 1 + args.size()));
			region.pushIdUse(image);
			region.pushIdUse(coords);
	...
	public:
		...
		SpirvBuilder(IdGenerator& idGenerator, std::size_t expInstructionsCount)
			...
			, memoryModelRegion{3}
			, entryPointRegion{1}
			, executionModeRegion{1}
			, debugRegion{0}
			, annotationRegion{1}
			, globalRegion{1}
			, functionDeclRegion{1}
		...
		template <typename T>
		requires(std::is_base_of_v<Type, T>)
			ConstantValue<TypeToValue<T>> createConstant(T type, std::span<const std::uint32_t> values)
		{
