...
struct ColorBuffer
{
	...
	void setRegister(unsigned index, std::uint32_t value)
	{
		switch (index)
		{
			...
			case CB_COLOR0_SLICE - CB_COLOR0_BASE:
			{ // SLICE
				auto sliceTileMax = GNM_GET_FIELD(value, CB_COLOR0_SLICE, TILE_MAX);
				// std::printf("  * TILE_MAX = %lx\n", sliceTileMax);
				break;
			}
			case CB_COLOR0_VIEW - CB_COLOR0_BASE:
			{ // VIEW
			...
			case CB_COLOR0_INFO - CB_COLOR0_BASE:
			{ // INFO
			...
			case CB_COLOR0_ATTRIB - CB_COLOR0_BASE:
			{ // ATTRIB
...
struct CacheBufferOverlay : CacheOverlayBase
{
	...
	{
		std::lock_guard lock(mtx);
		auto size = stride == 0 ? static_cast<std::uint64_t>(elementCount) * elementSize : static_cast<std::uint64_t>(elementCount) * stride;
		...
		if (entry->cacheMode != CacheMode::LazyWrite)
		{
			auto writeBackTask = createCpuTask([=, this](
				...
				if (entry->cacheMode == CacheMode::None)
				{
					...
					{
						taskChain->add([&] {
							return uploadBuffer->tryLock(tag, shader::AccessOp::None).isLocked ? TaskResult::Complete : TaskResult::Reschedule;
						});
					}
				}
				else
...
private:
		...
		std::uint64_t size)
	{
		...
		bufferOverlay->cacheMode =
			size >= 3 * bridge::kHostPageSize ? CacheMode::LazyWrite : (size >= bridge::kHostPageSize ? CacheMode::AsyncWrite : CacheMode::None);
		...
		bool isStorage)
	{
      ...
      bpp = 8;
			...
			.subresourceRange =
				{
					.aspectMask = static_cast<VkImageAspectFlags>(
						isColor ? VK_IMAGE_ASPECT_COLOR_BIT : VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT),
					.baseMipLevel = 0, // TODO
					.levelCount = 1,
					.baseArrayLayer = 0,
					.layerCount = 1,
		...
		newOverlay->cacheMode = memSize >= bridge::kHostPageSize * 3 ? CacheMode::LazyWrite : CacheMode::AsyncWrite;
...
struct Cache
{
	...
	getBuffer(std::uint64_t tag, TaskChain& initTaskChain, std::uint64_t address,
		...
		std::uint32_t elementSize, shader::AccessOp access)
	{
		auto& line = getLine(address, stride != 0 ? stride * elementCount : elementSize * elementCount);
			...
			switch (uniform.kind)
			{
				...
				case shader::Shader::UniformKind::Image:
				{
					...
					VkWriteDescriptorSet writeDescSet{
						...
						.descriptorCount = 1,
						.descriptorType =
							uniform.kind == shader::Shader::UniformKind::StorageImage ? VK_DESCRIPTOR_TYPE_STORAGE_IMAGE : VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
	...
	std::uint64_t indeciesAddress, std::uint32_t indexCount)
{
		 ...
		 auto& colorBuffer : regs.colorBuffers)
	{
		...
		colorBlendEquation.push_back(VkColorBlendEquationEXT{
			...
			.srcAlphaBlendFactor =
				regs.blendSeparateAlpha ? blendMultiplierToVkBlendFactor(regs.blendAlphaSrc) : blendMultiplierToVkBlendFactor(regs.blendColorSrc),
			.dstAlphaBlendFactor =
				regs.blendSeparateAlpha ? blendMultiplierToVkBlendFactor(regs.blendAlphaDst) : blendMultiplierToVkBlendFactor(regs.blendColorDst),
	...
	if (regs.depthEnable)
	{
		...
		depthAttachment = {
			...
			.storeOp = regs.depthWriteEnable && regs.zWriteBase ? VK_ATTACHMENT_STORE_OP_STORE : VK_ATTACHMENT_STORE_OP_DONT_CARE,
	...
	VkIndexType vkIndexType = (regs.indexType & 0x1f) == 0 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32;
	...
	if (needConversion)
	{
		...
		else
		{
			if (indexSize == 16)
			{
				...
				{
					auto [dstIndex, srcIndex] = converterFn(i);
					std::uint32_t origIndex = origIndexSize == 16 ? ((std::uint16_t*)indecies)[srcIndex] : ((std::uint32_t*)indecies)[srcIndex];
	...
	auto drawTaskFn = [colorAttachments = std::move(colorAttachments),
		...
		vkCmdSetFrontFace(drawCommandBuffer, regs.face ? VK_FRONT_FACE_CLOCKWISE : VK_FRONT_FACE_COUNTER_CLOCKWISE);
	...
	VkSemaphore signalSemaphore, VkFence fence)
{
	...
	imageRef = getCache().getImage(
		...
		buffer.tilingMode == 1 ? kTileModeDisplay_2dThin : kTileModeDisplay_LinearAligned,
