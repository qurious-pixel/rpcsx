	...
	orbis::Process* process)
{
	...
	{
		result->processParam =
			phdrs[sceProcParamIndex].p_vaddr ? reinterpret_cast<void*>(imageBase +
	...
	{
		result->moduleParam =
			phdrs[sceModuleParamIndex].p_vaddr ? reinterpret_cast<void*>(imageBase +
	...
	if (dynamicPhdrIndex >= 0 && phdrs[dynamicPhdrIndex].p_filesz > 0)
	{
		...
		auto sceStrtab = sceStrtabIndex >= 0 && sceDynlibDataPhdrIndex >= 0 ? reinterpret_cast<const char*>(
																				  image.data() + dyns[sceStrtabIndex].d_un.d_val +
																				  phdrs[sceDynlibDataPhdrIndex].p_offset) :
																			  nullptr;

		auto sceDynlibData =
			sceDynlibDataPhdrIndex >= 0 ? image.data() + phdrs[sceDynlibDataPhdrIndex].p_offset : nullptr;

		auto sceSymtabData =
			sceSymtabIndex >= 0 && sceDynlibData != nullptr ? reinterpret_cast<const Elf64_Sym*>(
		...
		for (auto dyn : dyns)
		{
			...
			switch (dyn.d_tag)
			{
				case kElfDynamicTypeScePltGot:
					result->pltGot =
						dyn.d_un.d_ptr ? reinterpret_cast<std::uint64_t*>(imageBase + dyn.d_un.d_ptr) : nullptr;
					break;
